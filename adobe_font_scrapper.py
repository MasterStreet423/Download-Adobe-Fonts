...
languages = {
    "english": ("Enter the URL or name of the Adobe font: ","Downloading: "),
    "spanish": ("Escriba la URL o el nombre de la fuente de Adobe: ","Descargando: ")
}

while True:
    try:
        language = input("Elige un idioma (Choose a language): english/spanish\n").lower()
        input_url_font = input(languages[language][0])
        ...
        def download_font(url:str):
            ...
            # Identificadores de los campos de nombre: Name Field Identifiers
            ID_FULL_NAME = 4  # Nombre completo de la fuente: Full name of the font
            ID_FAMILY_NAME = 1  # Nombre de la familia de la fuente: Font family name
            ID_POSTSCRIPT_NAME = 6  # Nombre PostScript de la fuente: PostScript name of the font
            name = get_name()
            for x in range(font_count):
                url = fonts[x]
                new_number = font_names[x]
                print(languages[language][1], new_number)
                r = requests.get(url)
                # font in otf
                font = fontTools.ttLib.TTFont(io.BytesIO(r.content))

                # Cambiar el nombre en la tabla 'name': Change the name in the 'name' table
                for record in font['name'].names:
                    if record.nameID in [ID_FULL_NAME, ID_FAMILY_NAME, ID_POSTSCRIPT_NAME]:
                        record.string = new_number.encode('utf-16-be')
                # save as otf
                os.makedirs("fonts", exist_ok=True)
                os.makedirs(f"fonts/{name}", exist_ok=True)
                font.save(f"fonts/{name}/{new_number}.otf")
        ...
        print("Su entrada no es válida (Your input isn't valid)")
